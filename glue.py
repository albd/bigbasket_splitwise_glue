...
items_clean = [re.match('^[\s~^#]*([^()]*\w)[\S\s]*$',elem).group(1) for elem in items]	#cleans the item list by stripping whitespaces, ^ and ~. The cleanup has been a prudent in that it removes only known junk.
